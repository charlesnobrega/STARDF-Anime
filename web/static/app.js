...
async function showDetails(item) {
    ...
    try {
        ...
        sources.forEach((src, idx) => {
            ...
            btn.style.cssText = "padding: 10px 18px; background: rgba(255,255,255,0.05); border: 1px solid var(--line); color: white; border-radius: 8px; cursor: pointer; font-weight: 600; transition: 0.3s; font-size: 0.85rem;";
...
async function playEpisode(ep, item, source, btn) {
    ...
    try {
        ...
        await fetch(`/api/play?url=${encodeURIComponent(data.url)}&referer=${encodeURIComponent(referer)}&title=${encodeURIComponent(title)}`);
